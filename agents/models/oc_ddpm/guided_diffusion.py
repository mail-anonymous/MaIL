...
class GuidedDiffusion(nn.Module):
    ...
    def guided_sample(self, state: torch.Tensor, goal, discriminator, start: int = 0.2, verbose=False, return_diffusion=False):
        """
        ...
        for i in reversed(range(0, self.n_timesteps)):
            ...
            x_cur = x.float().clone().detach().requires_grad_(True)
            x_denoised = self.p_sample(x, timesteps, state, goal)
            d_cur = (x_cur - x_denoised) / timesteps

            # Involve discriminator Guidance
            logits = discriminator(x_cur, timesteps)
            prediction = torch.clip(logits, 1e-6, 1 - 1e-6).requires_grad_(True)
            log_ratio = torch.log(prediction / (1 - prediction))

            discriminator_guidance = torch.autograd.grad(log_ratio.sum(), x_cur, retain_graph=False)[0]

            d_cur = d_cur + dg_factor * (discriminator_guidance / timesteps)
